...
async function bunjangDetailViaApi(pid) {
  ...
  return {
    ...
    seller: data?.shop || product?.shop || data?.seller || product?.seller
      ? (() => {
          const shop = data?.shop || product?.shop || data?.seller || product?.seller || {};
          return {
            nickname:
              shop.name ||
              shop.shop_name ||
              ...
              shop.userName ||
              '',
            reviewCount: shop.review_count ?? shop.reviewCount ?? shop.num_reviews,
            salesCount: shop.sale_count ?? shop.salesCount ?? shop.num_sell,
            reviewRating: shop.rating ?? shop.review_rating ?? shop.score,
          };
        })()
      : null,
    source: 'bunjang-api',
...
async function extractDetailFromDom(page, platform, finalUrl) {
  return page.evaluate(
    ({ plat, href }) => {
      ...
      const pickSellerString = (obj, keys) => {
        if (!obj || typeof obj !== 'object') return '';
        for (const key of keys) {
          const value = obj[key];
          if (typeof value === 'string' && value.trim()) return value.trim().slice(0, 40);
          if (value && typeof value === 'object') {
            const nested = pickSellerString(value, ['name', 'nickname', 'nickName', 'displayName', 'username', 'shopName']);
            if (nested) return nested;
          }
        }
        return '';
      };
      const sellerFromDaangnDom = () => {
        const txt = document.body?.innerText || '';
        const mannerM = txt.match(/(\d{2,3}(?:\.\d+)?)\s*°\s*C/i);
        const mannerScore = mannerM ? Number(mannerM[1]) : null;
        const nickEl =
          document.querySelector(
            '[class*="UserNickname" i], [class*="user-nickname" i], [class*="nickname" i], [data-testid*="user-name" i], [class*="UserName" i], a[href*="/kr/users/"], a[href*="/users/"]'
          ) || null;
        let nickname = String(nickEl?.textContent || '')
          .replace(/\s+/g, ' ')
          .trim();
        if (!nickname || nickname.length > 24 || /원$|채팅|관심|조회|공유|신고/.test(nickname)) nickname = '';
        if (!nickname) {
          const lines = txt
            .split('\n')
            .map((line) => line.trim())
            .filter(Boolean);
          const idx = lines.findIndex((line) => /\d{2,3}(?:\.\d+)?\s*°\s*C/i.test(line));
          if (idx > 0) {
            for (let i = idx - 1; i >= Math.max(0, idx - 5); i -= 1) {
              ...
              if (
                line.length >= 1 &&
                line.length <= 20 &&
                !/원$|끌올|시간|분\s*전|일\s*전|채팅|관심|조회|공유|신고|매너온도/.test(line)
              ) {
                nickname = line;
                break;
              }
        ...
        const location = String(
          document.querySelector('[class*="region" i], [class*="RegionName" i], [class*="user-region" i]')?.textContent ||
            ''
        )
          .replace(/\s+/g, ' ')
          .trim()
          .slice(0, 40);
        if (!nickname && mannerScore == null && !location) return null;
        return { nickname, mannerScore, location };
      };
      let seller = null;
      if (plat === 'daangn') {
        try {
          ...
          if (s >= 0) {
            ...
            if (e > js) {
              ...
              if (ld && typeof ld === 'object') {
                for (const val of Object.values(ld)) {
                  if (val?.product) remixProduct = val.product;
                  if (val?.user) remixUser = val.user;
                  if (val?.seller) remixUser = remixUser || val.seller;
                  if (val?.product?.user) remixUser = remixUser || val.product.user;
                  if (val?.product?.seller) remixUser = remixUser || val.product.seller;
                }
              }
            }
          }
          const jsonLdUser = (() => {
            for (const script of document.querySelectorAll('script[type="application/ld+json"]')) {
              try {
                ...
                const nodes = Array.isArray(parsed) ? parsed : [parsed];
                for (const node of nodes) {
                  const author = node?.author || node?.seller || node?.creator;
                  const name = pickSellerString(author, ['name', 'nickname', 'displayName']);
          ...
          const source = remixUser || remixProduct?.user || remixProduct?.seller || remixProduct?.author || jsonLdUser;
          const nickname =
            pickSellerString(source, ['nickname', 'nickName', 'name', 'displayName', 'username', 'shopName', 'userName']) ||
            pickSellerString(remixProduct, ['sellerName', 'userName', 'nickname']);
          const mannerScoreRaw =
            source?.mannerScore ??
            source?.temperature ??
            source?.mannerTemperature ??
            source?.score ??
            remixProduct?.mannerScore;
          const mannerScore = Number(mannerScoreRaw);
          const location = pickSellerString(source, ['regionName', 'region', 'location', 'address', 'areaName']);
          const domSeller = sellerFromDaangnDom();
          const mergedName = nickname || domSeller?.nickname || '';
          const mergedScore = Number.isFinite(mannerScore) ? mannerScore : domSeller?.mannerScore;
          const mergedLocation = location || domSeller?.location || '';
          if (mergedName || mergedScore != null || mergedLocation) {
            seller = {
              nickname: mergedName,
              mannerScore: mergedScore != null && Number.isFinite(mergedScore) ? mergedScore : null,
              location: mergedLocation,
            };
          }
        } catch {
          seller = sellerFromDaangnDom();
        }
      } else if (plat === 'bunjang') {
        const nickEl = document.querySelector('[class*="ShopName" i], [class*="shop-name" i], [class*="seller" i] a, a[href*="/shop/"]');
        const nickname = String(nickEl?.textContent || '')
          .replace(/\s+/g, ' ')
          .trim()
          .slice(0, 40);
        if (nickname) seller = { nickname, shopName: nickname };
      }
