  ...
  return page.evaluate(
    ({ plat, href }) => {
      ...
      const push = (raw) => {
        ...
        if (plat === 'joongna') {
          // Keep seller product photos only — drop logos, UI chrome, avatars, related thumbs.
          if (
            /icon|logo|\.svg|_next\/static|\/common\/Profile|profile_[a-z0-9_-]*\.(?:png|jpe?g|webp)|TradeType|badge|banner|favicon|app[-_]?install|qr[-_]?code/i.test(
              u
            )
          ) {
            return;
          }
          if (!/(?:img\d*\.)?joongna\.com|cloudinary|kakaocdn/i.test(u)) return;
          if (!/\/media\/original\//i.test(u)) return;
          if (/impolicy=thumb/i.test(u) && /size=(?:[1-9]|[1-9]\d|1[0-5]\d)\b/i.test(u)) return;
        }
        imageUrls.push(u);
      };
      if (plat === 'joongna') {
      ...
      if (!imageUrls.length || plat !== 'joongna') {
      ...
      const scoreJoongna = (u) => {
        let s = 0;
        if (/\/media\/original\//i.test(u)) s += 20;
        if (/impolicy=resize/i.test(u)) s += 3;
        if (/impolicy=thumb/i.test(u)) s -= 8;
        return s;
      };
      const deduped = [];
      ...
      if (plat === 'joongna') deduped.sort((a, b) => scoreJoongna(b) - scoreJoongna(a));
...
export async function importListingByUrl(rawUrl) {
  ...
  return withPage(async (page) => {
    ...
    await page.waitForTimeout(
      classified.platform === 'daangn' ? 3200 : classified.platform === 'joongna' ? 2800 : 2200
    );
