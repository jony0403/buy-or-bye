...
- **작성일:** 2026-05-19  
- **확장 버전:** `extension/manifest.json` → `2.5.2`  
...
| 확장 | `2.5.2` — 검색 탭 ID 저장 → 수집 끝나면 `background.js`가 탭 자동 닫기 |
...
- [ ] 검색어가 **한 줄 키워드**가 아니라 **설명 문장·프롬프트 잔여**로 번개 검색창에 들어가는 경우 있음 (예: `사진과 판매자 제목 모두 … 검색결과` 형태) → `analyzer-server.mjs` 출력 정제·JSON/한 줄 강제 재검토.
...
| **Gemini·유사 매물** | `/api/search-query`: 사진(`imageUrls`)·제목·본문 + **Gemini Google Search 연동**으로 제품 파악 후 검색어 한 줄. 실패 시 멀티모달만(`pipeline`: `google_search` \| `multimodal_fallback`). 프롬프트에 특정 브랜드 규칙 없음. |
...
*이 문서는 2026-05-20 세션까지 반영, 코드베이스 v2.5.2 기준. 수정 시 manifest version과 함께 갱신할 것.*
