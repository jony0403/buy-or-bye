...
**2026-09-16 제품 정리:** `/api/product-summary`는 식별(`product-identify`) 후 Google Search 조사(`product-info-research`)→JSON(`product-info-lookup`) 2호출로 설명·신품가·제조사 환각을 줄인다. Step3는 시세·비교필터가 둘 다 끝나야 다음 단계가 열리며, 한쪽이 먼저 끝나도 버튼 상태를 다시 갱신한다. 시세 카드는 AI 서술형 headline/`sourceNote`를 숨기고 고정 제목을 쓴다.
...
**디자인 기준:** 분석 웹 UI를 고칠 때는 루트의 `design.md`를 먼저 기준으로 삼는다. 컬러는 현재 오렌지/민트 계열을 유지하되 step별 accent를 다르게 쓰고, 좌측 레일·영수증 출력·Step 5 채팅 레이아웃 원칙도 이 문서에 맞춘다.
**노션 발표 정리(맥북에서 이어서 사용):** Notion 페이지 `중고 매물 분석기 - 프롬프트 작업 과정 정리`는 `기말프로젝트` 하위 페이지이며, URL은 `https://www.notion.so/3678e54a321781039482f64e9b4f091c`. 이미지 임베드는 GitHub public raw URL에 의존하므로 저장소를 private으로 되돌리면 깨진다. 대표 이미지 5장은 `assets/notion-presentation/` 아래 ASCII 파일명으로 따로 저장해 두었다. 상세는 §13.3·§15.
**현재 분석 웹 상태(2026-05-22):** 1단계 제품 정리 완료 후 2단계 제품 리스크가 자동 실행된다. 2단계는 `prompts/product-risk.txt` 자연어 조사 → `prompts/product-risk-json.txt` 카드 JSON 변환의 2호출 구조이며, 제품 진위/공식성 검증이 아니라 “이미 식별된 제품을 중고로 살 때 확인할 관련 이슈·고질병”만 다룬다. 2단계가 완료되기 전에는 3단계 DOM을 아예 렌더링하지 않는다. 3단계는 번개·당근·중고나라 비교 매물 수집과 AI 중고가이드까지만 담당하고, 최종 판단 영수증은 4단계로 분리한다. 5단계 판매자 대화 도우미는 아직 구현하지 않고 추후 작업으로 남겨 둔다. 판매자 이미지 분석은 홍보용·공식·쇼핑몰 스크랩 이미지를 실물 사진과 구분해, 홍보컷을 실물 하자 없음으로 단정하지 않도록 `analyzer-server.mjs` 프롬프트와 `analyzer/app.js` 배지 fallback을 보강했다. 디버그용 `AI 대화` 버튼은 헤더에서 직접 Gemini+Google Search 질의를 보낼 수 있다. **제품 식별:** 제목·사진의 세대/후속작 번호(`스위치2`, `Switch 2`, `OLED` 등)를 기본형으로 일반화하지 않도록 `product-identify.txt`·`product-summary.txt` 규칙 + `analyzer-server.mjs` `preserveVariantTokens()` 후처리로 보정한다. **프롬프트 원칙:** 테스트 매물이 무엇이든 특정 상품군·브랜드 예시를 프롬프트에 박지 말고, 모든 상품군에 통하는 “제품 동일성 식별자 vs 판매글/상태/거래 수식어 분리” 절차로만 보강한다.
**당근 비교 썸네일(2026-05-22, 검증됨):** 시세 확인 시 당근 탭은 `active: false`로 열어도 썸네일이 들어가야 한다. 검색 DOM의 `img[alt="thumbnail"]`·`/origin/article/` URL만 썸네일로 쓰고, 앱스토어 배지·광고·`og:image` 홍보 이미지는 버린다. 비활성 탭에서는 lazy-load가 안 되므로 `saveComps` 전에 `daangn.enhanceSearchListings()`로 상세 HTML fetch(Remix·HTML regex)까지 **await**한다. 카테고리 `/buy-sell/s/` 링크는 수집 제외. 상세는 **§7.2**.
...
**마지막 UI/확장 작업 상태(2026-06-02):** 발표는 끝났고 `2022194031전조운.pdf`, `presentation-prompts/presentation-script.md`는 사용자가 삭제했다. 분석 웹은 CSS-only `dashboard-v2` 톤으로 정리 중이며, `analyzer/app.js`는 Step 4 영수증 배경 높이가 영수증 출력과 같이 늘어나도록 조정했다. URL 붙여넣기 import 실험 코드는 `analyzer/index.html`·`analyzer/app.js`·`extension/background.js`·`extension/bridge-analyzer.js`에 들어갔지만 아직 불안정해서 폼은 `hidden` 상태다. 자세한 주의사항은 §9·§11·§13.4.
...
| **실매물 가격 참고** | LLM 추정 단독 ❌ → 번개·당근·중고나라 **비교 매물 URL**과 AI 중고가이드가 근거 |
...
```json
{
  "platform": "bunjang | daangn | joongna",
  "platformLabel": "번개장터 | 당근마켓 | 중고나라",
  "itemId": "숫자 또는 당근 id",
  "title": "",
  "price": 30000,
...
```json
{ "bunjang": true, "daangn": true, "joongna": true, "at": 1716123456789 }
```
...
**검색 목록 (비교 매물 썸네일 — 재발 방지 메모)**

- **카드 기준:** `[data-gtm="search_article"]` 안의 `a[href*="/buy-sell/"]` + slug-id (`DETAIL_PATH_RE`). **카테고리** `/kr/buy-sell/s/` 는 매물이 아니므로 **제외**.
- **썸네일 URL 조건:** `isDaangnArticleImageUrl` — 호스트에 `karrot`/`gcp-karroter`/`karrotmarket` 등 + 경로 `/origin/article/` (예: `https://img.kr.gcp-karroter.net/origin/article/...`). `alt="thumbnail"` 우선.
- **쓰지 말 것:** 앱스토어/구글플레이 배지, 당근 앱 홍보 `og:image`, 화면 전체 `nearbyImage` fallback, 광고·배너 URL. 분석 화면 `comparisonImageUrl()`도 동일 필터.
- **비활성 탭(`active: false`):** 검색 탭 DOM의 `img.src`가 비어 있어도 정상일 수 있음. 그래서 `harvestSearchListings` 직후 **`enhanceSearchListings`(`fillMissingSearchImages`)를 `tryAutoCollectSearch` / `COLLECT_SEARCH`에서 await** — 상세 페이지 `fetch` → `imageFromSearchHtml` / `imageFromRemixHtml` / HTML 내 `/origin/article/` regex.
- **저장:** `comps.js` merge 시 `imageUrl: ""` 이면 예전 잘못된 썸네일을 **지움**(재수집 시 배지 URL 잔류 방지).
- **진단(당근 검색 탭 콘솔):** `document.querySelectorAll('[data-gtm="search_article"]')` → `img[alt="thumbnail"]`의 `src`에 `/origin/article/` 있으면 DOM은 정상; 분석에 없으면 enhance/저장 타이밍 문제.
...
3. **[유사 매물 검색 · 자동 수집]**  
   - 검색어 = 제목 기반  
   - 번개·당근·중고나라 탭 `active: false` 로 생성  
...
- Step 1 매물 정리: 제목·가격·배송비·판매자·본문·사진(슬라이더)·원본 링크, 제품 정리 카드
- Step 2 리스크 판별: 제품 정리 완료 후 사용자가 시작. `product-risk.txt`로 자연어 조사하고 `product-risk-json.txt`로 관련 이슈/고질병 카드 JSON 변환, 판매글/판매자 이미지 분석 포함
- Step 3 가격 참고: **Step 2 완료 후에만 DOM 렌더링**. 번개·당근·중고나라 비교 매물 수집, AI 동일제품 판별, AI 중고가이드 표시. 비교 매물 0건이면 화면에 실패를 먼저 띄우지 않고 뒤에서 AI 검색어를 최대 2회 재생성해 재수집
- Step 4 최종 결론 영수증: Step 3의 비교 매물 필터와 AI 중고가이드가 끝난 뒤 렌더링. 영수증 출력/재출력은 이 단계에만 둔다
- Step 5 판매자 대화 도우미: **나중에 구현 예정**. 현재 코드에 UI를 만들지 않는다. 추후 Step 4 영수증 결과와 1~3단계 근거를 바탕으로 판매자 확인 질문/네고 문구를 만드는 단계로 계획
- 사진 클릭 → `#lightbox` (max viewport - 32px)  
- 「새로 분석」→ 현재 매물의 AI/비교/영수증 캐시를 지운 뒤 `MARKET_SCRAPE_REQUEST`

**2026-06-02 UI 메모**

- `analyzer/style.css` 하단에 CSS-only 대시보드 리프레시가 누적되어 있다. 효과를 더 얹기보다, 다음 리팩터에서는 기존 실험 오버라이드를 정리하고 레이아웃/토큰을 한 번에 재작성하는 편이 안전하다.
- Step 4 최종 CTA(`.stage-four-start-card`)는 보라→핑크→주황 그라데이션이 의도된 디자인이다. 전역 `background-image: none` 같은 규칙을 쓰면 이 버튼이 하얗게 죽으므로 금지.
- 영수증 출력은 `followPurchaseReceiptPrint()`가 scroll-follow를 담당하고, `updatePurchaseReceiptLayout({ animate: true })`가 현재 패널 높이에서 목표 높이로 늘어나게 만든다. 이 순서가 깨지면 스크롤식에서 배경이 먼저 길어져 화면이 아래에 고정되는 느낌이 난다.
- 제품 요약 카드의 제조사/판매처 줄은 잘리지 않도록 `.product-summary-text`·`.product-summary-top` 고정 높이를 CSS 하단에서 풀어 둔 상태다.
...
| 가격 참고 근거 | 번개·당근·중고나라 comps와 AI 중고가이드 |
...
| URL 붙여넣기 import 불안정 | `OPEN_LISTING_URL` 실험 코드가 있으나 아직 탭 로딩/파싱/저장 타이밍이 불안정해 분석 웹 폼은 `hidden`. 나중에 다시 개발할 때 확장 reload 후 플랫폼별 실제 URL로 테스트 |
| CSS 오버라이드 누적 | 2026-06-02 디자인 실험으로 `analyzer/style.css` 하단 오버라이드가 길어졌다. 다음 큰 UI 개편은 덧칠보다 정리/재작성 권장 |
...
**발표용 이미지 동기화:** 발표 캡처는 `발표용 이미지 캡처/` 아래 PNG·`대화첨부_이미지_목록.md`로 GitHub에 올린다(대화첨부 01~42, 스크린샷 약 25장, §16용 42번 포함). Cursor 임시 assets 경로만 있는 파일은 저장소 밖이라 `깃동기화`만으로는 안 올라간다 — 반드시 이 폴더에 복사 후 동기화. 다른 PC는 `git pull` 후 `발표용 이미지 캡처/`와 `presentation-prompts/prompt-process-summary.md`를 본다.

**노션 발표 정리(맥북용):**

- 페이지: [중고 매물 분석기 - 프롬프트 작업 과정 정리](https://www.notion.so/3678e54a321781039482f64e9b4f091c) — Notion `기말프로젝트` 하위 페이지.
- 용도: PPT 대신 노션에서 주제, AI 역할/단계, 작업 과정 이미지, 프롬프트 설계 포인트, 원본 프롬프트 링크/핵심 원문을 보여주는 발표·정리 문서.
- 이미지: 노션 이미지 블록은 `raw.githubusercontent.com` 공개 URL을 사용한다. 대표 이미지 5장은 `assets/notion-presentation/01-initial-analysis-ui.png` ~ `05-prompt-process-capture.png`에 ASCII 파일명으로 복사해 두었고, GitHub raw URL로 임베드했다.
- 중요: Notion 서버가 이미지 URL에 인증 없이 접근해야 하므로 GitHub 저장소가 public이어야 한다. 저장소를 private으로 되돌리면 노션 이미지가 404로 깨진다.
- 맥북에서 이어서 작업할 때: `git pull origin main` 후 `assets/notion-presentation/`, `발표용 이미지 캡처/`, `prompts/*.txt`를 기준으로 노션 내용을 보강한다. 노션 MCP를 쓸 경우 먼저 도구 스키마를 읽고, 필요하면 `plugin-notion-workspace-notion` 인증 후 `notion-fetch`/`notion-update-page`를 사용한다.
...
| 프롬프트 | 브랜드·제품별 하드코딩 규칙 **없음** — 웹·사진으로 제품 파악 후 번개·당근·중고나라용 키워드 |
...
| 키워드 후보 선택 | `2.5.9` — 플로팅 패널에서 `키워드 후보 만들기` 클릭 → AI가 `maxQueries: 3` 후보 반환 → 후보 버튼 클릭 시 해당 키워드로 번개·당근·중고나라 검색 탭 열고 자동 수집. 수집 완료 후 `background.js`가 탭 자동 닫기 |
...
| 중고나라 3번째 플랫폼 | `2.7.0` — `hosts/joongna.js` 추가. `web.joongna.com/product/{id}` 상세는 Next flight 데이터(`productTitle`, `productDescription`, `productPrice`, `parcelFeeYn`)와 DOM/meta fallback으로 읽고, `web.joongna.com/search/{keyword}` 검색 결과도 비교 매물로 자동 수집한다. `marketScrapeComps`와 자동 닫기 플래그는 `bunjang/daangn/joongna` 3개 플랫폼 기준으로 동작한다. |
| 당근 판매자 정보 보강 | `2.7.7` — 당근 상세에서 판매자 닉네임·지역이 Remix 데이터에 없을 때도 페이지 텍스트의 `닉네임 → 지역 → 매너온도` 블록을 읽어 `seller.nickname`·`seller.location`에 저장한다. 분석 웹 판매자 줄은 기존 `닉네임 · 매너온도 · 지역` 형식으로 표시된다. |
| 프롬프트 범용성 원칙 | 2026-05-21 — 테스트 매물 때문에 `피규어`, 특정 게임기/브랜드, 특정 카테고리 예시를 프롬프트에 넣지 않는다. 검색어·제품 요약 프롬프트는 화장품/인형/전자기기/의류/생활용품 등 어떤 상품군에도 적용되도록 “핵심 제품 식별자만 남기고 상태·거래·홍보·옵션 수식어는 필요할 때만 좁히기” 같은 범용 절차로 작성한다. 사용자는 코드 후처리보다 프롬프트 설계를 선호하지만, 프롬프트도 특정 사례 맞춤형으로 오염시키면 안 된다고 명시함 |
| 가격 참고자료 원칙 | 2026-05-21 — 관련 중고 매물이 충분히 많이 나오지 않는 경우가 많으므로 3단계를 “시세 확인”으로 약속하지 않는다. UI와 `purchase-receipt.txt`는 비교 매물을 구매 판단용 가격 참고자료로만 다루고, 영수증은 `최종 결론` 중심으로 표시한다. 영수증 직전에는 `used-price-guide.txt` 기반 AI 중고가이드가 번개·중고나라 등 국내 검색 결과와 수집 후보를 함께 보고 S/A/B급 같은 상태별 가격표를 만든다. |
...
| 분석 웹 디자인·URL import 실험 | 2026-06-02 — `analyzer/style.css`에 CSS-only dashboard-v2 톤을 적용하고 제품 요약 줄림을 완화했다. `analyzer/index.html`에는 URL import 폼이 있으나 `hidden`; `analyzer/app.js`·`extension/background.js`·`extension/bridge-analyzer.js`에는 URL을 백그라운드 탭으로 열어 `REFRESH_AND_SAVE`로 저장하는 실험 코드가 남아 있다. 아직 불안정하므로 UI는 숨겨 둔 상태에서 다음에 이어서 개발 |
...
- [ ] 프롬프트 수정 시 제품군·브랜드·현재 테스트 매물에 맞춘 예시를 추가하지 말 것. 화장품/인형/전자기기/의류/생활용품 등 모든 경우에 적용되는 범용 판단 절차로만 보강할 것.
...
- [ ] URL 붙여넣기 import 재개: 현재 폼은 `hidden`. 확장 background가 URL 탭을 열고 `REFRESH_AND_SAVE`로 저장하는 실험 코드는 있으나, 실제 플랫폼별 로딩/닫기/오류 UX 안정화 필요
- [ ] 분석 웹 CSS 정리: 2026-06-02 실험 오버라이드가 하단에 누적됨. 다음 큰 디자인 개편 전 `style.css` 토큰/레이아웃 정리 권장
- [ ] `docs/기획1.pdf` 추가  
...
| **노션 발표 정리** | https://www.notion.so/3678e54a321781039482f64e9b4f091c — `기말프로젝트` 하위 페이지. 이미지 raw URL은 `assets/notion-presentation/` 파일을 사용하며, GitHub 저장소가 public이어야 노션에서 보임 |
